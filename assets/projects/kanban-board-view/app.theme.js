...
function applyTheme(theme) {
    ...
    const active = ThemeToggle.querySelector(`.theme-${index}`);
    const current = ThemeToggle.querySelector('.theme-active');
    ...
    const remove = Object.values(Themes).map((t) => `theme-${t}`).filter((cls) => cls !== themeClass);
